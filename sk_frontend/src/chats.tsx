...
const Chats: React.FC = () => {
    ...
    return (
        <>
        <div className="bg-gray-200 dark:bg-slate-600 min-h-screen">
            <Header />
            <div className="h-[15vh] w-full flex items-start px-8 bg-gradient-to-r from-blue-500 to-indigo-500 dark:from-blue-800 dark:to-slate-900">
                <h1 className="text-3xl text-white font-bold mt-6 dark:text-gray-100">Chats</h1>
            </div>
            <div className="flex items-center justify-center">
                <div className="w-full max-w-7xl h-[80vh] rounded-lg shadow-lg lg:mt-[-11vh] flex">
                    <div className="w-2/6 bg-slate-50 dark:bg-slate-900 rounded-tl-lg rounded-bl-lg flex-col">
                        <div className="mt-4">
                            <div className="w-full flex justify-center items-center space-x-2">
                                <div className="w-[70%] flex">
                                    <div className="bg-slate-200 dark:bg-slate-500 rounded-bl-lg rounded-tl-lg flex items-center justify-center pl-2">
                                        <Search size={18} className="text-[#5a5858] dark:text-gray-200 cursor-pointer" />
                                    </div>
                                    <input type="text" placeholder="Search for a user" className="w-full h-8 bg-slate-200 dark:bg-slate-500 dark:text-gray-200 dark:placeholder:text-gray-200 rounded-tr-lg rounded-br-lg outline-none px-3 py-2 font-sans" 
                                    onChange={(e) => {
                        ...
                        {/* <div className="bg-gray-300 h-[0.5px] mt-2"></div> */}
                            ...
                            <div key={index} className={`w-full flex-col  ${currentChat._id === chat._id ? "bg-[#EAEAEA] dark:bg-gray-600" : "hover:bg-[#F0F0F0] dark:hover:bg-gray-600"} ` } onClick={() => messageUser(chat.users)}>
                                <div className="flex items-center space-x-4 p-2">
                                    ...
                                    <div className=" w-full">
                                        <div className=" w-full" >
                                            <h1 className="text-lg font-sans dark:text-gray-200">{chat.users[0].username !== loggedUsername ? chat.users[0].username : chat.users[1].username}</h1>
                                            <p className="text-sm text-gray-500 font-sans dark:text-gray-200">{chat.lastMessageSender === loggedUsername ? "You: " : !chat.lastMessageSender ? "" : chat.lastMessageSender+": "}{chat.lastMessage?.length+chat.lastMessageSender?.length > 29 ? chat.lastMessage.slice(0,29-chat.lastMessageSender?.length)+"..." : chat.lastMessage}</p>
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <div className="w-full flex justify-end">
                                    <div className="bg-[#F0F0F0] dark:bg-gray-600 h-[0.5px] w-[85%] "></div>
                                </div>
                            </div>
                            )):
                            <div className="w-full h-full p-4 flex flex-col space-y-3 animate-pulse">
                                {
                                    Array(6).fill(0).map((_, index) => (
                                        <div key={index} className="w-full flex-col dark:bg-gray-600">
                                            <div className="flex items-center space-x-4 p-2">
                                                <div className="p-1">
                                                    <div className="h-11 w-11 rounded-full bg-slate-300 overflow-hidden flex items-center justify-center">
                    ...
                    {currentChat._id ? (
                    <div className="w-4/6 rounded-tr-lg rounded-br-lg">
                        <div className="bg-[#F0F0F0] dark:bg-slate-800 h-[10%] flex items-center space-x-4 px-3 rounded-tr-lg" >
                            <div className="">
                                <div className="h-11 w-11 rounded-full bg-slate-300  overflow-hidden flex items-center justify-center">
                                    <img src="/images/user.png" alt="avatar" className="h-6 w-6 object-cover" />
                                </div>
                            ...
                            <div className="flex-col justify-center items-center ">
                                <p className="text-lg font-sans dark:text-gray-200">{currentChat?.users[0]?.username !== loggedUsername ? currentChat?.users[0]?.username : currentChat?.users[1]?.username}</p>
                                { (isTyping && typingChatId === currentChat?._id) ? 
                                    <p className="text-sm text-gray dark:text-gray-200">typing...</p> : 
                                    <p className="text-sm text-gray dark:text-gray-200">{currentChat?.users[0]?.username !== loggedUsername ? currentChat?.users[0]?.userRole : currentChat?.users[0]?.userRole === loggedRole ? currentChat?.users[1]?.userRole : currentChat?.users[0]?.userRole}</p>
                                }                            
                            </div>
                            <div className=" w-full flex justify-end">
                                <DropdownMenu>
                                    <DropdownMenuTrigger className="outline-none">
                                       <div className="rounded-full h-8 w-8 flex items-center justify-center cursor-pointer hover:bg-gray-300 dark:hover:bg-slate-500">
                                           <EllipsisVertical size={20}  className="text-gray-500 dark:text-gray-300"/> 
                                       </div>
                                    </DropdownMenuTrigger>
                        ...
                        {!loading ? 
                        <div className={`bg-[#EAE6DF] dark:bg-slate-500 h-[80%] overflow-auto p-2`} ref={messagesEndRef}>
                            {messages.map((message: any, index) => (
                                message?.sender?.username === loggedUsername ? (
                                    <div key={index} className="flex justify-end my-2">
                                        <div className="bg-blue-500 dark:bg-blue-800 px-3 py-2 rounded-lg max-w-lg flex gap-2">
                                            ...
                                            <div className="flex justify-end items-end gap-1">
                                                ...
                                                <div className="flex justify-end items-end">
                                                    <CheckCheck size={16} className={message.readBy?.some((user: { username: string; userRole: string }) => user.username !== message.sender.username || user.userRole !== message.sender.userRole) ? "text-white " : "text-blue-900 dark:text-gray-800"} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div key={index} className="flex justify-start ml-2 my-2">
                                        <div className="bg-gray-300 px-3 py-2 rounded-lg max-w-lg flex gap-2">
                                            <div className=" break-words overflow-hidden font-sans text-gray-900">
                                                {message.message}
                                            </div>
                                            <div className="flex justify-end items-end gap-1">
                        ...
                        :
                        <div className="w-full p-4 flex flex-col space-y-3 bg-[#EAE6DF] dark:bg-slate-500">
                            {
                                Array(9).fill(0).map((_, index) => (
                                    <div key={index} className={index%2 == 0 ? `flex justify-end my-2` : `flex justify-start my-2`}>
                                        <div className="">
                                            <Skeleton style={{ height:40, width:250}} className="dark:bg-slate-900"/>
                                        </div>
                                    </div>
                                ))
                            }
                            <div>
                                <Skeleton style={{ height:45}} className="dark:bg-slate-900"/>
                            </div>
                        ...
                        {!loading ?
                        <div className="w-full h-[10%]  flex justify-center items-center bg-[#F0F0F0] dark:bg-slate-800 space-x-2 rounded-br-lg">
                            <textarea
                                placeholder="Type a message..."
                                className="w-[80%] h-[70%] rounded-md outline-none resize-none px-3 py-2 font-sans dark:bg-slate-400 dark:placeholder:text-gray-900 dark:text-gray-900"
                                onChange={(e) => {
                            ...
                            />
                            <SendHorizonal size={24} className="text-[#5a5858] cursor-pointer dark:text-slate-400" onClick={sendNewMessage} />
                        </div>
                        :null
                        }
                    </div>):
                    (
                        <div className="w-4/6 bg-gray-200 dark:bg-slate-800  text-white rounded-br-lg rounded-tr-lg shadow-lg flex flex-col justify-center items-center p-8">
                        ...
                        {/* Welcome Message */}
                        <h1 className="text-4xl font-bold text-gray-700 font-sans text-center drop-shadow-md animate-fade-in dark:text-gray-300">
                            Welcome to Skillcase
                        </h1>
                    
                        {/* Subtitle */}
                        <p className="text-lg text-center text-gray-600 text- font-medium font-sans mt-2 animate-fade-in dark:text-gray-400 dark:animate-fade-in">

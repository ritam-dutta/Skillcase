...
const Login : React.FC<Login> = ({})=>{
    ...
    const handleLogin = async () => {
        setErrorMessage("")
        try {
            const response = await axios.post("http://localhost:8000/api/v1/freelancer/freelancer_login", {
                username: username,
                password: password,
            });
      ...
      <div className="container h-lvh w-lvw flex flex-col justify-between items-center bg-cover bg-center gap-0 overflow-hidden bg-[url(../images/background.jpg)] ">
        <div className="navbar h-[9-vw] w-lvw relative flex flex-row justify-between items-center">
            ...
            <div className="sign h-[100%] w-[20%] flex flex-row justify-end items-center mr-10">
                {/* <button className="btn_sign hover:bg-[rgb(241,186,6,0.7)] h-[80%] w-[40%] bg-transparent rounded-[30px] border-[3px] border-[rgb(241,186,6)] transition-colors duration-500 ease-in-out "><Link className="head_text text-lg font-sans text-[rgb(241,186,6)] text-[17px] relative btn_sign-hover:text-black" to="/login">Sign In</Link></button> */}
                <button className="btn_sign hover:bg-[rgb(241,186,6,0.7)] h-[80%] w-[40%] bg-transparent rounded-[30px] border-[3px] border-[rgb(241,186,6)] transition-colors duration-500 ease-in-out "><Link to="/register" className="head_text text-lg font-sans text-[rgb(241,186,6)] text-[17px] relative">Sign Up</Link></button>
            </div>
        </div>
        ...
        <div className="login w-[27vw] h-[60vh] rounded-[5px] border-[1px] border-black back backdrop-blur-xl">
            ...
            <div className="last mt-[3%] w-[100%] h-[10%] flex flex-col items-center justify-end">
                <p className="font-serif text-[rgb(255,225,2)]" >Don't have an account? <Link to="/register" className="text font-sans text-[#a1f6fc] text-base" >Sign Up</Link></p>
            </div>
        </div>

...
<div className="h-[85vh] w-screen bg-[url('/images/background.jpg')] bg-cover bg-center flex flex-col justify-between">
  ...
  <div className="h-[15vh] w-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center px-4 sm:px-8">
    <h1 className="text-3xl sm:text-4xl text-white font-bold">Followings</h1>
  </div>

  {/* Main Content */}
  <main className="h-[72vh] flex-grow w-full flex justify-center items-start py-6 px-4 sm:px-8">
    {!loading ? (
      <div className="w-full sm:w-11/12 lg:w-3/5 bg-slate-50 shadow-lg rounded-lg p-6 sm:p-8 mt-4 lg:mt-[-13vh]">
        {/* Form Content */}
            ...
            <div>
              ...
              {clientFollowings.length > 0 ? (
                clientFollowings.map((following) => (
                  <div key={following._id} className="flex items-center justify-between bg-slate-100 p-4 shadow-md mb-4">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-gray-300 p-2">
                        <img src="/images/user.png" alt="" className="h-5 w-5" />
                      </div>
                      <div>
                        <Link to={`/client/profile/${following.username}`} className="text-xl">
                          @{following.username}
                        </Link>
                      </div>
                    </div>
            ...
            <div className="mt-6">
              ...
              {freelancerFollowings.length > 0 ? (
                freelancerFollowings.map((follower) => (
                  <div key={follower._id} className="flex items-center justify-between bg-slate-100 p-4 shadow-md mb-4">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-gray-300 p-2">
                        <img src="/images/user.png" alt="" className="h-5 w-5" />
                      </div>
      ...
      <div className="w-full sm:w-11/12 lg:w-3/5 bg-slate-50 shadow-lg rounded-lg p-6 sm:p-8 mt-4 sm:mt-[-13vh]">
        <div className="space-y-6">

...
const Client_reg : React.FC<Client_reg> = ({})=>{
    ...
    const handleRegister = async () => {
        ...
        if(!email || !username || !fullname || !dob){
            alert("Please fill all the required fields")
            return;
        }
        try {
            const response = await axios.post("http://localhost:8000/api/v1/client/client_register", {
                username: username,
                password: password,
                email: email,
                dob: dob,
    ...
    <>
        ...
        <div className="navbar h-[9vh] w-[100vw] relative flex flex-row justify-between items-center">
            ...
            <div className="sign h-[100%] w-[20%] flex flex-row justify-evenly items-center">
                <button className="btn_sign hover:bg-[rgb(241,186,6,0.7)] h-[80%] w-[40%] rounded-[30px] bg-transparent border-[3px] border-[rgb(241,186,6)] transition duration-500 ease-in-out"><Link to="/login" className="head_text text-lg font-sans text-[rgb(241,186,6)] text-[17px] relative">Sign In</Link></button>
        ...
        <div className="login w-[27vw] h-[65vh] border-[1px] border-black rounded-[5px] backdrop-blur-[25px]">
            ...
            <div className="last w-[100%] h-[10%] flex flex-col items-center justify-start">
                <p className="font-sans text-[rgb(255,225,2)]">Already have an account? <Link to="/login" className="text font-sans text-[#a1f6fc] text-lg" >Sign In</Link></p>
            </div>
        </div>
        <Footer></Footer>
    </div>

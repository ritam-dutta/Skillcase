...
const SavedProjects: React.FC = ({}) => {
    ...
    useEffect(() => {
        const fetchSavedProjects = async () => {
            ...
            try {
                ...
                const allSavedProjects = projects.filter((project: SavedProjects) => fetchedSavedProjects?.includes(project._id));
    ...
    const applyProject = (id: string, employer: string ) => {
        try {
            setProjects(projects.filter((project: SavedProjects) => project._id !== id));
    ...
    <div className="lg:h-[18vh] sm:h-[10vh] w-full bg-gradient-to-r from-blue-500 to-indigo-500 dark:from-blue-800 dark:to-slate-900 flex items-center lg:items-start px-8">
        <h1 className="text-3xl text-white dark:text-gray-100 font-bold lg:mt-6">Saved Projects</h1>
    </div>
                    ...
                    <div className="flex justify-between align-center">
                        ...
                        <div className="flex flex-col justify-start items-center space-y-4">
                            ...
                            <p className="dark:text-blue-400 text-sm text-blue-700 font-sans font-medium">
                                {formatCount(projects.find((pr: SavedProjects) => pr._id === project._id)?.likedBy.length ?? 0)}
                            </p>
                            </div>
                        </div>
                    </div>
